# JunkOS Support & FAQ

**For: Website Support Page & App Store Listing**

---

## Contact Information

**Email:** support@junkos.app  
**Phone:** 1-800-JUNK-OS1 (1-800-586-5671)  
**Hours:** Monday-Friday 8:00 AM - 8:00 PM EST, Saturday-Sunday 9:00 AM - 6:00 PM EST  
**Website:** https://www.junkos.app/support  
**In-App Support:** Settings → Help & Support → Contact Us

**Emergency/Same-Day Support:** For urgent booking issues on the day of your scheduled pickup, call our priority line at 1-800-586-5671 and press 1.

---

## Frequently Asked Questions

### Getting Started

**Q: How does JunkOS work?**  
A: JunkOS makes junk removal simple: (1) Take photos of your items, (2) Get an instant AI-powered price estimate, (3) Choose your pickup time, (4) Our vetted hauler shows up and handles everything. Payment is processed securely through the app after completion.

**Q: What items can I get hauled away?**  
A: We accept furniture, appliances, electronics, mattresses, yard waste, construction debris, general household junk, and more. We cannot accept hazardous materials (paint, chemicals, asbestos), medical waste, or items requiring special permits.

**Q: How accurate are the AI estimates?**  
A: Our AI estimates are accurate within ±10% in most cases. For best results, take clear photos from multiple angles showing all items. The final price is confirmed when the hauler arrives and may adjust slightly based on actual volume or accessibility.

**Q: Is there a minimum charge?**  
A: Yes, our minimum pickup charge is $89, which covers up to ¼ truck load. This includes labor, hauling, and disposal fees.

---

### Booking & Scheduling

**Q: Can I get same-day pickup?**  
A: Yes! Subject to availability in your area. Check the app for same-day time slots. We recommend booking at least 24 hours in advance for guaranteed availability.

**Q: How do I reschedule or cancel?**  
A: You can reschedule or cancel up to 4 hours before your pickup time at no charge through the app (Bookings → Select Booking → Reschedule/Cancel). Cancellations within 4 hours or no-shows are subject to a $35 fee.

**Q: What if I'm not home during pickup?**  
A: As long as the items are accessible (garage, driveway, curb), we can complete the pickup without you present. Mark your booking as "Unattended Pickup" and provide access instructions in the notes.

**Q: Do you service my area?**  
A: JunkOS currently serves [list major metropolitan areas]. Enter your ZIP code in the app to check availability. We're expanding rapidly—sign up for notifications if we're not in your area yet!

---

### Pricing & Payment

**Q: What's included in the price?**  
A: Your quote includes labor, hauling, transportation, disposal fees, and recycling costs. The price you see is the price you pay—no hidden fees, fuel surcharges, or last-minute add-ons.

**Q: What payment methods do you accept?**  
A: We accept all major credit cards (Visa, Mastercard, American Express, Discover), debit cards, Apple Pay, and Google Pay. Payment is processed securely after your pickup is complete.

**Q: Can I get a refund?**  
A: If you cancel more than 4 hours before your scheduled pickup, you'll receive a full refund. If there's an issue with your service, contact support within 24 hours and we'll work with you to resolve it, which may include a partial or full refund.

**Q: Do you offer discounts?**  
A: Yes! First-time users receive $25 off their first haul. We also offer discounts for recurring pickups and bulk bookings. Check the app for current promotions.

---

### Service & Safety

**Q: Are your haulers licensed and insured?**  
A: Absolutely. All haulers on the JunkOS platform are vetted, licensed, and carry liability insurance. We perform background checks and verify credentials before approving any service provider.

**Q: Can I track my hauler?**  
A: Yes! You'll receive real-time tracking once your hauler is on the way. See their location, estimated arrival time, and driver information in the app.

**Q: What if the hauler is late or doesn't show up?**  
A: We take punctuality seriously. If your hauler is running late, you'll receive a notification with an updated ETA. If they're more than 30 minutes late without notice, contact support immediately for priority assistance or rebooking at no charge.

**Q: What happens to my items after pickup?**  
A: We prioritize eco-friendly disposal. Usable items are donated to local charities, recyclable materials go to proper recycling facilities, and remaining waste is disposed of responsibly. You'll see a disposal summary in your completion receipt.

---

### Technical & App Issues

**Q: The app isn't giving me a price estimate. What should I do?**  
A: Try these steps: (1) Ensure photos are clear and well-lit, (2) Take photos from multiple angles showing all items, (3) Check your internet connection, (4) Update to the latest app version. If issues persist, contact support with your photos and we'll provide a manual estimate within 2 hours.

**Q: I didn't receive a notification/confirmation.**  
A: Check that notifications are enabled: Settings → Notifications → JunkOS → Allow Notifications. Also verify your email address in Profile settings. Contact support if you still don't receive confirmations.

**Q: The app crashes when I try to take photos.**  
A: Update the app to the latest version and ensure JunkOS has camera permissions (Settings → Privacy → Camera → JunkOS). If the issue continues, try restarting your phone or contact support.

**Q: How do I delete my account?**  
A: Go to Settings → Account → Delete Account. Note: this permanently removes your data, booking history, and saved payment methods. Contact support if you need assistance.

---

## Common Issues & Solutions

### Issue: "Upload Failed" when submitting photos

**Solutions:**
- Check your internet connection (switch between WiFi/cellular)
- Reduce photo file size: tap "Optimize Photos" before uploading
- Close and reopen the app
- Try uploading one photo at a time instead of multiple
- Contact support if problem persists

---

### Issue: No available time slots showing

**Solutions:**
- Try expanding your search window (select different days)
- Check if you're searching during peak times (weekends fill up fast)
- Enable "Notify Me" to get alerts when new slots open
- Consider booking further in advance for guaranteed availability
- Contact support for priority scheduling assistance

---

### Issue: Price seems too high/low

**Solutions:**
- Review the item list and volume estimate in your quote
- Ensure all photos clearly show the items and their size
- Check that you've selected the correct load size (¼, ½, ¾, full truck)
- Use the "Get Manual Estimate" option for complex loads
- Contact support to review your estimate before booking

---

### Issue: Hauler marked items as "inaccessible"

**Solutions:**
- Ensure items are in an easily reachable location (ground level preferred)
- Clear pathways and remove obstacles
- Add access notes in booking (gate codes, parking instructions)
- If you have stairs/difficult access, note this during booking
- Contact support if you believe the hauler's assessment was incorrect

---

### Issue: Payment declined or failed

**Solutions:**
- Verify your card details are correct and up-to-date
- Check that your card has sufficient funds/credit
- Contact your bank—some flag junk removal charges as unusual
- Try a different payment method
- Contact support for payment assistance

---

## Still Need Help?

**Email us:** support@junkos.app  
**Call us:** 1-800-JUNK-OS1 (1-800-586-5671)  
**In-app chat:** Settings → Help & Support → Chat with Support (Mon-Fri 8 AM - 8 PM EST)

For urgent issues on the day of your pickup, call our priority hotline and press 1 for immediate assistance.

**Response Times:**
- **Email:** Within 4 hours during business hours, 24 hours on weekends
- **Phone:** Immediate during business hours, voicemail outside hours
- **In-app chat:** 5-15 minutes during business hours

---

## Report a Safety Concern

If you experienced unsafe behavior from a hauler or have a safety concern about a service provider, please contact us immediately at safety@junkos.app or call 1-800-586-5671 and press 2. We take safety reports very seriously and investigate all claims within 24 hours.

---

## Leave Feedback

We're always improving! Share your suggestions:
- **In-app:** Settings → Send Feedback
- **Email:** feedback@junkos.app
- **App Store Review:** We read every review and use your input to make JunkOS better

Thank you for choosing JunkOS! 🚀
