...
export function normalizeDriverJob(job) {
  ...
  return {
    ...
    requires_acceptance: job.status === "assigned",
    driver_payout: typeof job.driver_payout === "number" ? job.driver_payout : null,
    payout_status: job.payout_status ?? null,
    volume_adjustment_proposed: Boolean(job.volume_adjustment_proposed),
    has_open_change_order: Boolean(job.has_open_change_order),
    completion_pin_set: Boolean(job.completion_pin_set),
    volume_estimate: typeof job.volume_estimate === "number" ? job.volume_estimate : null,
    version: job.version,
    address: job.address ?? "",
    city: job.city ?? "",
