...
FLAGS = {
    ...
    "inbound_callback_menu": {"default": True, "desc": "When nobody picks up, offer the caller a callback from a person (press 1) before handing them to Maya (press 2)"},
    "missed_booking_capture": {"default": True, "desc": "When a call is priced and the caller agrees but no job is created, put them on the desk as a callback (internal task only, never texts the customer)"},
